...
BOOL File::Open(const WCHAR *pszFileName, const OpenMode enMode, const CharCode enCharCode)
{
	BOOL bCode = FALSE;
	UINT uOpenFlag = 0;
	int  nRetry = 5;
...
BOOL File::UnlinkFolder(const WCHAR *pszPath, const BOOL bParentUnlink)
{
...
{
	DWORD dwbAttr = GetFileAttributesW(pszFileName);
...
{
	DWORD dwbAttr = GetFileAttributesW(pszFilePath);
